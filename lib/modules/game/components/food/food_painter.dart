// lib/app/modules/game/components/food/food_painter.dart
...
class FoodPainter extends PositionComponent {
  ...
  // A reference to the camera that is viewing our world.
  final CameraComponent cameraToFollow;
  double _animT = 0.0;

  // The constructor now requires the camera reference.
  FoodPainter({required this.foodManager, required this.cameraToFollow});

  @override
  void update(double dt) {
    super.update(dt);
    // subtle pulsing animation for pellets
    _animT += dt;
  }
  ...
  void render(Canvas canvas) {
    super.render(canvas);
    // --- THIS IS THE FIX ---
    // We now get the visible area from the CORRECT camera.
    final visibleRect = cameraToFollow.visibleWorldRect;

    // Loop through the food data and only draw the ones on screen.
    for (final food in foodManager.foodList) {
      if (visibleRect.contains(food.position.toOffset())) {
        final floatScale =
            0.85 +
            0.15 *
                (0.5 + 0.5 * math.sin(_animT * 4.0 + food.position.x * 0.01));
        ...
        final paint = Paint()
          ..shader =
              RadialGradient(
                ...
                Rect.fromCircle(center: food.position.toOffset(), radius: r),
              );
